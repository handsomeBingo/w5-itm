...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	_processModuleDependencies(module, callback) {
		...
		const processDependency = (dep, index) => {
			// 构建 moduelGraph: 所谓 graph 应该就是模块间的亲代关系构成的树状图吧
			// dep 是 module.dependencies 或者 dep.blocks 中的各种依赖
			// currentBlock 则是当前的 module 模块
	...
	finish(callback) {
		...
		// 触发 hooks.finishModules 钩子
		// -> ResoverCachePlugin: 计算实际 resolver 和 使用缓存中的 resolve 结果的比例
		// -> InferAsyncModulesPlugin: 处理 buildMeta.async 为 true 的模块（为使用全局 await 的模块标记）
		// -> FlagDependencyExportsPlugin: 标记导出的 plugin 有点长，是为处理 treeshaking 的准的，把所有 es6 模块的 export 信息都收集起来，在 moduleGraph 上就能够得到各个模块的导出信息
		this.hooks.finishModules.callAsync(modules, err => {
			// 这个回调中就是干了点收集模块的报错和警告的工作
	...
	seal(callback) {
		...
		if (this._backCompat) {
			// 建立 module 和 chunkGraph 联系：为 module 设置 chunkGraph
			for (const module of this.modules) {
				ChunkGraph.setChunkGraphForModule(module, chunkGraph);
			}
		}

		// -> WarnCaseSensitiveModulesPlguin: 警告通过大小写字母区分不同模块的行为
		this.hooks.seal.call();

		this.logger.time("optimize dependencies");

		// 传入 compilation.modules 触发 compilation.hooks.optimizeDependencies 钩子
