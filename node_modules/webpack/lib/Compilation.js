...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	seal(callback) {
		...
		// -> FlagDependencyUsagePlugin: 递归标记所有依赖对模块导出的引用情况
		// -> FlagAllModulesAsUsedPlugin: 不触发
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
		...
		// 暂无插件：参数是 compilation.chunks 和 compilation.chunkGroups
		this.hooks.afterOptimizeChunks.call(this.chunks, this.chunkGroups);

		// 暂无插件：参数是 compilation.chunks, compilation.modules
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			...
			// ModuleConcatenationPlugin: 将多个模块合并成一个模块，减少 HTTP 请求数量和模块体积
			this.hooks.optimizeChunkModules.callAsync(
				...
				err => {
					...
					// ChunkModuleIdRangePlguin:
					// DeterministicModuleIdsPlguin: 的作用是为生成的模块根据模块内容生成确定性的 ID。确定性的模块 ID 对于长期缓存和优化构建非常有益，因为它们可以确保在重新构建时，相同的模块会得到相同的 ID，从而避免缓存失效。
					// HashedModuleIdsPlugin: 未触发
					// NamedModuleIdsPlugin: 未触发
					// NaturalModuleIdsPlugin: 未触发
					// OccurrenceModuleIdsPlugin: 未触发
					...
					// RecordIdsPlugin：遍历 record.json 文件获取 usedId，感觉是重用的 chunkId
					this.hooks.reviveChunks.call(this.chunks, this.records);

					this.hooks.beforeChunkIds.call(this.chunks);

					// DeterministicChunkIdsPlugin: 主要作用是为生成的代码块（chunk）生成确定性的、稳定的标识符。
					// NamedChunkIdsPlugin:
					// NaturalChunkIdsPlugin:
					// OccurrenceChunkIdsPlugin:
					this.hooks.chunkIds.call(this.chunks);

					// FlagIncludedChunksPlugin: 标记 chunk 是否存在包含关系
					...
					if (shouldRecord) {
						// RecordIdsPlugin.js 更新 reocords.modulea.usedIds 数组
						// SyncModuleIdsPlugin.js 未触发
						this.hooks.recordModules.call(this.modules, this.records);

						// RecordIdsPlugin.js records.chunks.usedIds
						this.hooks.recordChunks.call(this.chunks, this.records);
					}

					// MangleExportsPlugin.js 它的作用是对导出的模块进行命名混淆
	...
	createChunkAssets(callback) {
		...
		asyncLib.forEachLimit(
			...
			(chunk, callback) => {
				...
				let manifest;
				try {
					manifest = this.getRenderManifest({ // 触发 compilation.hooks.renderManifest 钩子
						chunk,
						hash: this.hash,
					...
					(fileManifest, callback) => {
						...
						assetCacheItem.get((err, sourceFromCache) => {
							...
							try {
								...
								this.hooks.chunkAsset.call(chunk, file); // HotModuleReplacementPlugin
