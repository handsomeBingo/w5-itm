	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap("SplitChunksPlugin", compilation => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				chunks => {
					// SplitChunksPlugin 有一个很重要的功能是提取公共模块到一个新的 chunk 是之前 CommonsChunkPlugin 的替代产物
					// 所以这里面一定有一个核心思路是找到那些被 chunk 共享的模块们，然后给他们新建一个 chunk
					...
					const groupedByExportsMap = new Map(); // key 是 模块，value 是引用 key 所对应模块的 chunk
					...
					for (const module of compilation.modules) {
						...
						const getCombsByUsedExports = memoize(() => {
							...
							// groupedByUsedExports 是引用 module 的 chunk 数组
						...
						for (const cacheGroupSource of cacheGroups) {
							...
							for (const chunkCombination of combs) {
								...
								// 是 optimize.splitChunks.cacheGroup.default.minChunks 最小 chunk 数的生效原理：
								// 即模块被不小于 cacheGroup.minChunks 个 chunk 引用时才单独拆分成一个 chunk
					...
					for (const [key, info] of chunksInfoMap) {
						if (removeMinSizeViolatingModules(info)) { // removeMiniSizeViolatingModules 把违反 minSize 的模块移除掉，这些模块不够拆分成一个 chunk，这里这个 violating 差点意思，应该叫 lowerThanMinSize 就好了，小于 minSize 的都移除掉
					...
					while (chunksInfoMap.size > 0) { // 到这里 chunkInfoMap 里面还有东西的话，这里面的就是应该被拆分成单独 chunk 的
						...
						if (newChunk === undefined) { // 如果没有命中复用的 chunk 则需要新建 chunk 了
					...
					// 确保满足最大体积要求，其实很简单，超过最大体积就再拆一遍，直到满足体积要求
