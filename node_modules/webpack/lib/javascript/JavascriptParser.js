...
class JavascriptParser extends Parser {
	...
	walkClass(classy) {
		...
		if (classy.body && classy.body.type === "ClassBody") {
			// body.body 就是 class 声明后面大括号中的内容
			for (const classElement of /** @type {TODO} */ (classy.body.body)) {
				if (!this.hooks.classBodyElement.call(classElement, classy)) {
					...
					if (classElement.value) {
						...
						// StaticBlock 是一个新的提案，用于在 class 主体内访问类和示例的属性，语法就是 static { this.xxx  }
						const wasTopLevel = this.scope.topLevelScope;
						this.scope.topLevelScope = false;
	...
	preWalkStatement(statement) {
		...
		// 触发 hooks.preStatement 钩子 <SyncBailHook> ：以下插件 work 时如果返回 true 则中断这个过程：
		// -> CompatibilityPlguin: 预处理嵌套 FunctionDeclaration 类型的语句，处理嵌套的 __webpack_require__ ? 啥叫嵌套 require? 你的代码里面写的 function __webpack_require__ () {} 这种就叫做嵌套的 require，这相当于是 webpack 的保险机制，如果你的代码里面的函数也叫做这个名字，webpack 会给你重命名 成 __nested_webpack_require_；如果命中这种条件返回 true 中断预处理；
		// -> InnerGraphPlugin: 预处理 FunctionDeclaration 类型的函数声明语句，为函数声明的，根据函数名和当前 parser.state 生成一个 Symble 标记，保存在 statementWithTopLevelSymbol 这个 WeakMap 中，最后返回 true 中断后续的解析逻辑；目测这个标记是给后面的内容准备的；

		if (this.hooks.preStatement.call(statement)) {
			this.prevStatement = this.statementPath.pop();
			return;
		}
		// 上面的 hooks.preStatement 预处理了 statement.type 为 FunctionDeclaration 的函数声明中的以下两种情况：
		// 1. 函数名叫做 __webpack_require__ 的，所谓 nested webpack_require
		// 2. 为函数声明的函数作 statementWithTopLevleSymbol 标记，干啥暂时未知。。。

		switch (statement.type) {
			case "BlockStatement":
				// BlockStatement 块语句则是用 {} 声明的块作用域，包括 do while 循环、if 条件之类的；严格模式不让使用 with 语句。。。。
				// BlockStatement 递归调用 preWalkStatement 方法
				this.preWalkBlockStatement(statement);
				break;
			case "DoWhileStatement":
				// DoWhileStatement 递归调用 preWalkStatement 方法
				this.preWalkDoWhileStatement(statement);
				break;
			case "ForInStatement":
				// 先调用 this.preWalkVariableDeclaration() 处理 for in 循环中的变量声明部分
				// 再调用 this.preWalkStatement() 预处理循环体中的内容
				this.preWalkForInStatement(statement);
				break;
			case "ForOfStatement":
				// 先处理顶层 await（state.topLevelScope 为 true），这是因为 for of 支持一个 for await of 的语法。for await of 语法原则上应该在 async 函数中使用，因此需要做顶层 await 检测
				// 核心触发 hooks.topLevelAwait 钩子进而触发 HarmonyDetectionParserPlugin 插件，主要使用顶层 await 这一实验特性标志是否打开未打卡则抛出异常，另外还检测仅允许在 esm 中使用这一特性；
				// 接着调用 this.preWalkVariableDeclaration() 处理常规的变量声明
				// 最后调用 this.preWalkStatement() 处理循环体
				this.preWalkForOfStatement(statement);
				break;
			case "ForStatement":
				// 如果有 statement.init 调用 this.preWalkStatement(statement.init) 先处理 for 循环中初始化的部分 statement.init
				// 再调用 this.preWalkStatment() 处理循环体
				this.preWalkForStatement(statement);
				break;
			case "FunctionDeclaration":
				// InnerGraphPlugin 在 hook.preStatement 中只处理了 topLevel 顶层的函数声明，非顶层的还是要被函数处理的
				// 调用 this.preWalkFunctionDeclaration 处理函名字（this.defineVariable 声明变量）
				this.preWalkFunctionDeclaration(statement);
				break;
			case "IfStatement":
				// 预处理 if 语句的语句体，处理 alternate, 就是 else if else
				// if () {} IfStatement
				// else if () {} statement.alternate
				// else {} statement.alternate.alternate
				this.preWalkIfStatement(statement);
				break;
			case "LabeledStatement":
				// Labeled 语句使用 preWalkStatment 方法接着预处理语句体
				this.preWalkLabeledStatement(statement);
				break;
			case "SwitchStatement":
				// 处理 switch case 语句，内部调用 this.preWalkSwitchCases() 处理
				// 内部遍历 statement.cases，把每一条 case 交给 this.preWalkStatement 方法
				this.preWalkSwitchStatement(statement);
				break;
			case "TryStatement":
				// 分别对 try catch finally 语句调用 this.preWalkStatement 方法分别处理
				this.preWalkTryStatement(statement);
				break;
			case "VariableDeclaration":
				// 预处理变量声明：这里面有点复杂，其实就是预处理各式各样的变量声明包括解构赋值之类的语法
	...
	blockPreWalkStatement(statement) {
		...
		switch (statement.type) {
			...
			case "ExportAllDeclaration":
				// export * from './a' 这种叫做 ExportAllDeclaration
				// 触发 hooks.exportImport  钩子
				// 触发 hooks.exportImportSpecifier 钩子
				// 进而触发 HarmonyExportDependencyParserPlugin 收集依赖 HarmonySideEffectExpotIm
				this.blockPreWalkExportAllDeclaration(statement);
				break;
			case "ExportDefaultDeclaration":
				// ExportDeaultDeclaration 处理的 export default 的语法
				// 首先处理 export default 后面的内容，接着交给 this.preWalkStatement() 预处理一下
				// 再交给 this.blockPreWalkStatement(statement.declaration) 处理声明中的内容
				// 最后判断 statement.declaration.type 不是 Function 声明也不是 Class 声明的情况：添加 HarmonyExportImportedSpecifierDependency 或者 HarmonyExportSpecifierDependency
				this.blockPreWalkExportDefaultDeclaration(statement);
				break;
			case "ExportNamedDeclaration":
				// 根据 named 有没有 source 添加不同依赖：
				// 有 source 比如 './a/b' HarmonyImportSideEffectDependency，这个是从别的模块来的所有有 sideEffect 副作用，估计这个都是在给 tree shaking 做准备吧
				// 没有 source 的，比如 export { x, y } 添加 HarmonyExportHeaderDependency 依赖
				this.blockPreWalkExportNamedDeclaration(statement);
				break;
			case "VariableDeclaration":
				// 处理 let 和 const 声明的 class 或者 function 表达式，如不是类或函数表达式则当普通变量处理一下
	...
	walkStatement(statement) {
		...
		// 触发 SideEffectFlagPlugin: 标记 sideEffectStatement 副作用语句？后面感觉可以专门开一个讲 tree-shaking 的专题
		// 讲 tree-shaking 的 博文：https://yo-cwj.com/2023/07/17/webpack-tree-shaking-related-config-and-process/
		// InnerGraphPlugin:
		...
		switch (statement.type) {
			case "BlockStatement":
				// block 的重新执行 blockPreWalkStatement 收集依赖
				// 接着递归调用它 this.walkStatements() 应该核心还是在查找各种依赖，比如 import() 、require 之类的
	...
	blockPreWalkExportNamedDeclaration(statement) {
		let source;
		if (statement.source) {
			// export {x, y} from './a' 这种叫做有 source 的
			source = statement.source.value;
			// HarmonyImportSideEffectDependency
			this.hooks.exportImport.call(statement, source);
		} else {
			// export { x, y } 这种则是对应没有 source 的
	...
	_preWalkVariableDeclaration(statement, hookMap) {
		// const x = 1;
		// declarator: { id: { type: 'Identifier', name: 'x', start: end: }, init: {}, type: 'VariableDeclarator', start: 6, end: 10 } 就是 x 这个变量名字
	...
	walkExpression(expression) {
		// 解析表达式中对具体模块的应用到的变量，把那些从模块导出的变量变成 webpack 的包装变量，
		switch (expression.type) {
	...
	enterPattern(pattern, onIdent) {
		...
		switch (pattern.type) {
			case "ArrayPattern":
				...
				break;
			case "AssignmentPattern":
				// const { x2 = 200 } = obj; 这种叫做 AssignmentPattern
	 ...
	 */
	parse(source, state) {
		// 非常一段时间以来我对 webpack parse 阶段存在很大的误解，我原以为它解析 JS 是为了理解 JS，或者提供钩子让大家在这个阶段修改 webpack 的源代码。其实 webpack 设计 parser 这个阶段是通过解析代码收集依赖用的，我们常说 webpack 会递归收集依赖这个过程就是这么来的。
		// 还有一个问题，为什么设计这么多钩子呢？这个设计就是 webpack 支持自定义机制了，甚至很多钩子都可以覆盖掉 webpack 原有的收集依赖的逻辑。 parser.hooks 很多都是 SyncBailHooks 同步带保险的钩子，一旦有一个回调 返回了 true 就会中断。当然，我们更多是扩展某一种收集依赖的方式，这里有一个比较恰当的例子是 MpxWebpackPlugin 里面为 require.aync() 收集依赖的处理。webpack 本身不支持 requier.asyn() 这个语法，通过分析可以得到 async() 方法中的是一个依赖。
		let ast;
		let comments;
		...
		// 触发 hooks.program: 触发以下插件
		// -> HarmonyDectectionParserPlguin：根据是否是 javascript/esm 或者 是否使用 import/export 增加 HarmonyCompatibilityDependency
		// -> CompatibilityPlugin: 判断开头不是这种 #! 文档声明的，添加 ConstDependency
		// -> UseStrictPlugin: 如果开头有 'use strick' 声明则添加 ConstDependency 目的是删掉原有的 use strict 稍后会在 render 阶段添加
		// -> InnerGraphPlugin: InnerGraph.enable(parser.state) 也不知道 enable 了个锤子 😂
		// -> SideEffectsFlagPlugin: 每次进行 program 时先讲 sideEffectStatement 这变量置空；

		if (this.hooks.program.call(ast, comments) === undefined) {
			// 检测 'use strict' 或者 'use asm' 这种文档声明
			this.detectMode(ast.body);

			// Pre walking iterates the scope for variable declarations
			// 预先遍历所有的作用域处理变量声明：
			// 1. 是给for / for in /for of 的变量声明了
			// 触发 hooks.preStatement 钩子 <SyncBailHook> ：以下插件 work 时如果返回 true 则中断这个过程：
			// -> CompatibilityPlguin: 预处理嵌套 FunctionDeclaration 类型的语句，处理嵌套的 __webpack_require__ ? 啥叫嵌套 require? 你的代码里面写的 function __webpack_require__ () {} 这种就叫做嵌套的 require，这相当于是 webpack 的保险机制，如果你的代码里面的函数也叫做这个名字，webpack 会给你重命名 成 __nested_webpack_require_；如果命中这种条件返回 true 中断预处理；
			// -> InnerGraphPlugin: 预处理 FunctionDeclaration 类型的函数声明语句，为函数声明的，根据函数名和当前 parser.state 生成一个 Symble 标记，保存在 statementWithTopLevelSymbol 这个 WeakMap 中，最后返回 true 中断后续的解析逻辑；目测这个标记是给后面的内容准备的；
			...
			// Block pre walking iterates the scope for block variable declarations
			// Block 预遍历作用域，处理块变量声明
			// 啥玩意叫 Block 呢：以下这几种，为啥呀叫 Block ；import export class 变量声明
			// => type == 'ImportDeclaration':
			// 遇到 import: 触发 hooks.import ，触发 HarmonyImportDependencyParserPlugin：添加 HarmonyImportSideEffectDependency
			// 遇到 importSpecifer (就是导入 import { x } from 'module' 这种)：parser.tagVariable() 方法标记变量
			// 不触发 CommonJsImportsParserPlugin: 添加 ConstDependency，是的，根本没注册上，被 445行代码半路return 。。。。。
			// => type == 'exportAllDeclaration':
			// 遇到 export * 触发 hooks.exportImport 钩子：
			// -> HarmonyExportDependencyParserPlugin
			// 1. 添加 ConstDependency，HarmonyImportSideEffectDependency
			// ->
			this.blockPreWalkStatements(ast.body);
			this.prevStatement = undefined;

			// walkStatements 的核心是遍历除了 import 、extport 语句的其他表达式语句中对其他模块变量的引用
			// 在 walkExpression 的过程中，解析表达式中的变量添加 HarmonyImportSpecifierDependency 依赖，把 import 进来的变量变成 webpack 包装变量
			// 处理上面 walkExpression 中，触发 ImportParserPlguin
			// 如果 type 是 importExpression 就会处理 import(a/b) 这种懒加载模块的逻辑，为拦截的模块收集异步依赖module.bolcks，类型为 AsyncDependenciesBlock
