...
class JavascriptGenerator extends Generator {
	...
	generate(module, generateContext) {
		const originalSource = module.originalSource(); // module 在 build 阶段就确定了 source
		if (!originalSource) {
	...
	sourceModule(module, initFragments, source, generateContext) {
		...
		// 所谓 prensentationalDependencies 就是描述性的依赖，原来不太懂现在大概懂了就是 ConstDendepency/HarmonyCompatibilityDependency 之类的
		// 并不直接对应原有的业务代码，而是针对运行时的辅助，比如增加一些代码或者删除一些代码
	...
	sourceDependency(module, dependency, initFragments, source, generateContext) {
		...
		const template = generateContext.dependencyTemplates.get(constructor); // Template 是和 constructor 绑定的，类型决定模板
		if (!template) {
			throw new Error(
